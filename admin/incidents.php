...
$sql = "SELECT i.*, c.category_name, u.fullname AS reporter
        FROM incidents i
        JOIN categories c ON i.category_id = c.category_id
        JOIN users u ON i.user_id = u.user_id
        WHERE " . implode(' AND ', $where) . "
        ORDER BY
          FIELD(i.priority,'High','Medium','Low'),
          FIELD(i.status,'Open','Assigned','In Progress','Resolved','Closed'),
          i.created_at DESC";
...
function status_class(string $s): string { return match($s) { 'Open'=>'s-open','Assigned'=>'s-assigned','In Progress'=>'s-progress','Resolved'=>'s-resolved','Closed'=>'s-closed',default=>'' }; }
...
<form method="GET" class="filter-bar">
  <input type="search" name="q" value="<?= htmlspecialchars($search) ?>" placeholder="Search incidents…">
  ...
  <a href="<?= SITE_URL ?>/admin/incidents.php" class="btn btn-sm" style="background:var(--line);">Clear</a>
  <?php endif; ?>
</form>
  ...
  <a class="incident-row clickable <?= status_class($inc['status']) ?>"
    ...
    <div class="inc-info">
      ...
      <div class="inc-meta">
        ...
        <span>·</span><span><?= date('d M Y', strtotime($inc['created_at'])) ?></span>
      </div>
    </div>
